// Sprint 1 — DB unification: 6 bảng PG thay 6 collection Mongo.
...
exports.up = async (knex) => {
  await knex.raw(`
    ...
    -- 4. role_permissions — 1 row / role (bảng permissions cũ có purpose khác)
    ...
    -- 6. real_estate_history — partitioned monthly by created_at (DECISIONS B2)
  ...
  await knex.raw(`SELECT ensure_future_history_partitions(2)`);
};

exports.down = async (knex) => {
  await knex.raw(`DROP FUNCTION IF EXISTS drop_old_history_partitions(INT)`);
  await knex.raw(`DROP FUNCTION IF EXISTS ensure_future_history_partitions(INT)`);
  await knex.raw(`DROP FUNCTION IF EXISTS create_history_partition(INT, INT)`);
  // Partition con DROP cùng parent qua CASCADE.
  await knex.raw(`DROP TABLE IF EXISTS real_estate_history CASCADE`);
  await knex.raw(`DROP TABLE IF EXISTS real_estate_details CASCADE`);
  await knex.raw(`DROP TABLE IF EXISTS role_permissions CASCADE`);
  await knex.raw(`DROP TABLE IF EXISTS subscriptions CASCADE`);
  await knex.raw(`DROP TABLE IF EXISTS import_errors CASCADE`);
  await knex.raw(`DROP TABLE IF EXISTS settings CASCADE`);
};
